...
5. **Never Commit**: NEVER commit changes unless explicitly asked by the user. The user needs to review changes before they are committed.
...
6. **Separate concerns**: utility functions and helpers belong in the `lib/` folder; constants and static values belong in the `constants/` folder
